...
# #######################################################
auth = Auth(session, db, define_tables=True, extra_fields = [Field('manager_id', 'reference auth_user', requires=IS_EMPTY_OR(IS_IN_DB(db, "auth_user", lambda r:f"{r.first_name}{r.last_name}")))])
